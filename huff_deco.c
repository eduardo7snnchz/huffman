...
void huffman_decoder(FILE * ent, FILE * sal, FILE *freq);
...
int main(int argv, char** argc){
	...
	if(!(freq = fopen(argc[3], "rb"))) {
		perror("ERRROR opening input file");
		return -1;
	}
    if(!(sal = fopen(argc[4], "wb"))){
		perror("ERROR opening output file");
		return -1;
	}

	if(*argc[1] == 'd')
		huffman_decoder(ent, sal,freq);
	else
		printf("Uso: %s [d] input output\n", argc[0]);
...
void huffman_decoder(FILE * ent, FILE * sal, FILE *freq){
	...
	if(fread(freqTable, sizeof(unsigned int), 256, freq) < 256) // Lectura de tabla de frecuencias
	{
		printf("Can't decode file.\n");
		exit(-1);
	}

	while(!feof(freq)) //Iterar por el flujo de entrada
		freqTable[fgetc(freq)]++; // Incrementar el contador para el caracter.
