...
void huffman_coder(FILE * ent, FILE * sal,FILE *freq);
...
int main(int argv, char** argc)
{
	...
	if(*argc[1] == 'c')
		huffman_coder(ent, sal,freq);
	else
		printf("Uso: %s [c] input output\n", argc[0]);
...
void huffman_coder(FILE * ent, FILE * sal, FILE *freq){
	...
	fwrite(freqTable, sizeof(unsigned int), 256, freq); // Escribir tabla de frecuencias
